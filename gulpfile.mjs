...
// ✅ Задача imagesAvif обновлена (фильтрация + обработка ошибок)
gulp.task('default', gulp.series(
  cleanDist,
  gulp.parallel(html, styles, scripts, fonts, imagesAvif, imagesSvg, favicon),
  gulp.parallel(server, watcher)
));

gulp.task('build', gulp.series(
  cleanDist,
  gulp.parallel(html, styles, scripts, fonts, fontsWoff, fontsSvg, images, imagesWebp, imagesAvif, imagesSvg, favicon),
));